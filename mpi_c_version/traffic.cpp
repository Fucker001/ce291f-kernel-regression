...
void calcSVD(double *in,double*out,unsigned int dim,int rank){
    double simulation_time = read_timer( );
		...
		dgesvd('O','S',dim,dim,in,dim,S,U,dim,Vt,dim,&info);
		// if (rank == 1){
			// printf("S:\n");
			// printMat(S,1,dim);
		// }
		...
		*/
		for(int i=0;i<dim;i++){
			for(int j=0;j<dim;j++){
				// out[j+i*dim] = S[i]*U[i+j*dim];
				out[j+i*dim] = S[i]*in[j+i*dim];
			}
		}
        // divide by RHO
        for(int i=0;i<dim*dim;i++){
		   out[i] = out[i] / pow(RHO,2);
        }
...
void calcPolyKernel(double *in, double *out, unsigned int iDim, unsigned int jDim, double c1,double c2, double d,int rank){
        ...
        calcSVD(intermed,out,maxDim,rank);
...
void calcGaussKernel(double *in,double *out,unsigned int iDim, unsigned int jDim,double sigma,int rank){
    double simulation_time = read_timer( );
	...
	*/
	...
	// printf("PreNormalization:\n");
	// printMat(intermed,maxDim,maxDim);
...
void calcLinKernel(double *in,double *out,unsigned int iDim,unsigned int jDim,int rank){
        unsigned int maxDim = max(iDim,jDim);
...
void getOTminusKtU(double *result,double *u, int Nt, int Np,double *rMajor, double *output, int N ){
/*
	printf("getOTminusKtU: OUTPUT: \n");
	printMat(output,1,N);
	printf("\n");
	
	
	printf("getOTminusKtU: rMajor: \n");
	printMat(rMajor,Np,N);
	printf("\n");
	
	
	printf("getOTminusKtU: u: \n");
	printMat(u,1,Np);
	printf("\n");

	printf("N:%d, Nt:%d, Np:%d\n\n\n\n",N,Nt,Np);
	
	dgemv('N',Nt,Np,1.0,rMajor,Nt,u,1,1.0,result,1);
	*/
	///*
	// printf("ENTERING GETOTMINUSKTU\n\n");
	for(int i=0;i<Nt;i++){
		for(int j=0;j<Np;j++){
			result[i] += rMajor[i*Np+j]*u[j];
			// printf("i: %d, j: %d, rm:%f, u[j]:%f\n",i,j,rMajor[i*Np+j],u[j]);
		}
	}
	//	*/
	for(int i=0;i<Nt;i++){
		result[i] = output[i]-result[i];
	}
}
...
void getAlpha(double *Alpha,double *u, int Np,double *rMajor,double *output,int Nt,int Kdim){
        double *L2 = (double*) calloc(Nt,sizeof(double));
        //matrix*vector to calculate right hand side, once only
        getOTminusKtU(L2, u,Nt, Np, rMajor,output, Kdim );
...
double phiOfU(double *U,int Np,double *rMajor,double *output,int Nt,int Kdim){
        double uNorm=0;
        for(int i=0;i<Np;i++){
            uNorm += abs(U[i]);
        }
        double rightHandSide;
        getAlpha(&rightHandSide,U,Np,rMajor,output,Nt,Kdim);
...
double Obj(double *originalU,double *uToGetWrittenTo, int mSteps,int Np, double *Gradient, double Delta_u,double *rMajor,double *output, int Nt,int Kdim){
        // returns { Phi( u-(Grad(u)*m*Delta_u) ) }
        updateUWithGrad(originalU,uToGetWrittenTo,Np,Gradient,mSteps,Delta_u);
        return phiOfU(uToGetWrittenTo,Np,rMajor,output,Nt,Kdim);
}
//run l1 optimization
void calcL1(double *u,int Np,double *cMajor,double *rMajor,double *output,int Nt,int Kdim,int rank,int *cancel){
	...
	getAlpha(&Alpha, u, Np,rMajor,output,Nt,Kdim);
	...
	while( *cancel == 0 ){ 
			if (PRINTSTATEMENTS){
					...
					printf("R: %d phi: %f\n",rank,phiOfU(u,Np, rMajor, output, Nt, Kdim));
					printf("R: %d Alpha: %f\n",rank,Alpha);
					printf("R: %d Beta:\n",rank);
					// printMat(Beta,1,Np);
			}
			...
			Delta_u = max(Delta_u, 0.00001);
			
			if (PRINTSTATEMENTS){
				printf("R: %d Delta_u: %f\n",rank,Delta_u);
			}
			// m = Obj(u,mSteps,Delta_u);
			m = Obj(u,newTempUSpace, mSteps,Np, Gradient, Delta_u,rMajor,output, Nt,Kdim);
			// mPlusOne = Obj(u,mSteps+1,Delta_u);
			mPlusOne = Obj(u,newTempUSpace, mSteps+1,Np, Gradient, Delta_u,rMajor,output, Nt,Kdim);
    ...
    MPI_Comm_rank( MPI_COMM_WORLD, &rank );
        ...
        for(unsigned int i=rank;i<numKernels;i+=n_proc){
                ...
                if(kernAss[i] == 0){
                        ...
                        calcLinKernel(input,workSpace+(workedSoFar*maxDimTotalLength),dimension,numInputPoints,rank);
                }else if(kernAss[i] == 1){
                        // printf("R:%d calculating gaussian %d\n",rank,i);
                        calcGaussKernel(input,workSpace+(workedSoFar*maxDimTotalLength),dimension,numInputPoints,gaussianParam[paramIndex],rank);
                }else if(kernAss[i] == 2){
                        // printf("R:%d calculating poly %d\n",rank,i);
                        calcPolyKernel(input, workSpace+(workedSoFar*maxDimTotalLength),dimension,numInputPoints, polyC1[paramIndex],polyC2[paramIndex], polyD[paramIndex],rank);
                }
                workedSoFar++;
        }
        if (PRINTSTATEMENTS){
                ...
                for(int x=0;x<ceil(1.0*maxDim*numKernels/blockSize);x++){
                        for(int i=y*blockSize;i<y*blockSize+blockSize&&i<maxDim;i++){
                                for(int j=x*blockSize;j<x*blockSize+blockSize&&j<maxDim*numKernels;j++){
                                        fatKRowMajor[j+i*maxDim*numKernels] = fatKColumnMajor[i+j*maxDim*numKernels];
                                }
                        }
                }
        } 
		*/
		...
		for(int i=0;i<maxDim;i++){
			for(int j=0;j<Np;j++){
				fatKRowMajor[j+i*Np] = fatKColumnMajor[i+j*maxDim];
			}
		}
        ...
        if (rank != 0){
                ...
                srand(rank+time(NULL)); //unique seed for each proc, same seed=same starting locations...
                for(int i=0;i<Np;i++){
                        //Begin with random starting location
                        U[i] = rand() % 20-10;//(1.0*rand()+1)/RAND_MAX; //0.1;
                }
                // if (PRINTSTATEMENTS && rank == 1){
                        // printf("My U: %f %f %f %f\n",U[0],U[1],U[2],U[3]);
                // }
				...
				// printf("R:%d Ending calcL1:\t%f\n",rank,read_timer( )-simulation_time);
                ...
                double *holder = (double*) calloc (n_proc*(Np),sizeof(double));
                MPI_Request *requests = (MPI_Request*) calloc (n_proc,sizeof(MPI_Request));
                for(int i=1;i<n_proc;i++){
                        // set up nonblocking recvs...
                        MPI_Irecv(holder+i*Np, Np, MPI_DOUBLE, i, L1RESULT, MPI_COMM_WORLD, &(requests[i]) );
                }
                // if (PRINTSTATEMENTS){
                ...
                // if (PRINTSTATEMENTS){
                        printf("First index: %d after %u iters, %f sec\n",c,iter,read_timer( )-simulation_time);
                // }
                int cancel = 1;
                MPI_Request *requests2 = (MPI_Request*) calloc (n_proc,sizeof(MPI_Request));
                for(int i=1;i<n_proc;i++){
                        // tell slaves to stop
				...
				// printf("Holder\n");
				// printMat(holder,n_proc,Np);
				// printf("\n\n");
				
                //Assume U now holds the optimal answer from some proc
                memcpy(U, &(holder[c*Np]),Np*sizeof(double));
                
                phiStar = phiOfU(U,Np,fatKRowMajor,output,rowsInTraining,maxDim);
                double vStar=0;
                getAlpha(&vStar,U, numKernels*maxDim,fatKRowMajor,output,rowsInTraining,maxDim);
        ...
        if (rank == 0){
                ...
                int success=0;
                dgetrf( rowsInTraining, rowsInTraining, KTrainingStar, rowsInTraining, pivotInfo, &success );
                ...
                dgetri( rowsInTraining,KTrainingStar,rowsInTraining,pivotInfo,&success);
                
                if (PRINTSTATEMENTS){
                        // printf("Ending inverse computation: %f\n",read_timer( )-simulation_time);
                        // printf("new KTrainingStar inverted:\n");
                        // printMat(KTrainingStar,rowsInTraining,rowsInTraining);
                        // printf("\n");
                        // printf("\n");
                }
                ...
                //compute KTrainingStar * OutputTrainingTranspose
                dgemv('N',rowsInTraining,rowsInTraining,1.0,KTrainingStar,rowsInTraining,
